...
static void _proc(struct bplus_tree *tree, char op, int n)
{
    ...
    switch (op) {
        case 'i':
            value.start = n;
            sprintf(value.fingerprit, "fp-%d", n);
//            strcpy(value.fingerprit, sprintf("fp-%d", n));
            bplus_tree_put(tree, n, value);
            break;
        case 'r':
            bplus_tree_rem(tree, n);
            break;
        case 's':
            value = bplus_tree_get(tree, n);
            printf("key:%d start:%d fingerprint: %s\n", n, value.start, value.fingerprit);
            break;
        default:
            break;
    }
...
static void debug(struct bplus_tree *tree)
{
    value_t value;
    value.start = 0;
    value.length = 9;
    sprintf(value.fingerprit, "fp-1");
    bplus_tree_put(tree, value.start, value);


    value.start = 10;
    value.length = 90;
    sprintf(value.fingerprit, "fp-2");
    bplus_tree_put(tree, value.start, value);

    value.start = 101;
    value.length = 9;
    sprintf(value.fingerprit, "fp-3");
    bplus_tree_put(tree, value.start, value);

    value.start = 111;
    value.length = 109;
    sprintf(value.fingerprit, "fp-4");
    bplus_tree_put(tree, value.start, value);

    value_t ret;
    ret = bplus_tree_get_fuzzy(tree, 40);
    printf("start:%d length:%d fp:%s\n", ret.start, ret.length, ret.fingerprit);
}

static void get_range(struct bplus_tree *tree, key_t key)
{
