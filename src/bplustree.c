...
static value_t bplus_tree_search(struct bplus_tree *tree, key_t key)
{
    value_t *ret;
    ret = NULL;
...
static key_t leaf_split_left(struct bplus_tree *tree, struct bplus_node *leaf,
                             struct bplus_node *left, key_t key, value_t data, int insert)
{
    /* split = [m/2] */
...
static key_t leaf_split_right(struct bplus_tree *tree, struct bplus_node *leaf,
                              struct bplus_node *right, key_t key, value_t data, int insert)
{
    /* split = [m/2] */
    int split = (leaf->children + 1) / 2;
...
static int leaf_insert(struct bplus_tree *tree, struct bplus_node *leaf, key_t key, value_t data)
{
    /* Search key location */
 ...
 * @return 0
 */
static int bplus_tree_insert(struct bplus_tree *tree, key_t key, value_t data)
{
    struct bplus_node *node = node_seek(tree, tree->root);
    while (node != NULL) {
        if (is_leaf(node)) {
...
struct bplus_tree *bplus_tree_init_debug(int fd, int block_size)
{
    ...
    _max_entries = (block_size - sizeof(node)) / (sizeof(key_t) + sizeof(value_t));
    if (_max_order <= 2) {
...
//    int fd = open(strcat(tree->filename, ".boot"), O_RDWR, 0644);
    ...
    _max_entries = (_block_size - sizeof(node)) / (sizeof(key_t) + sizeof(value_t));
