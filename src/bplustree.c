...
static int key_binary_search(struct bplus_node *node, uint64_t target)
{
    uint64_t *arr = key(node);
...
static inline int parent_key_index(struct bplus_node *parent, uint64_t key)
{
                                 ...
                                 struct bplus_node *r_ch, uint64_t key, int insert)
{
    ...
    memmove(&key(left)[0], &key(node)[0], pivot * sizeof(uint64_t));
    memmove(&sub(left)[0], &sub(node)[0], pivot * sizeof(off_t));

    /* replicate from key[insert] to key[split - 1] in original node */
    memmove(&key(left)[pivot + 1], &key(node)[pivot], (split - pivot - 1) * sizeof(uint64_t));
    ...
    memmove(&key(node)[0], &key(node)[split - 1], (node->children - 1) * sizeof(uint64_t));
                                   ...
                                   struct bplus_node *r_ch, uint64_t key, int insert)
{
    ...
    uint64_t split_key = key(node)[split - 1];
    ...
    memmove(&key(right)[pivot + 1], &key(node)[split], (right->children - 2) * sizeof(uint64_t));
                                   ...
                                   struct bplus_node *r_ch, uint64_t key, int insert)
{
    ...
    uint64_t split_key = key(node)[split];
    ...
    memmove(&key(right)[0], &key(node)[split + 1], pivot * sizeof(uint64_t));
    ...
    memmove(&key(right)[pivot + 1], &key(node)[insert], (_max_order - insert - 1) * sizeof(uint64_t));
...
static void non_leaf_simple_insert(struct bplus_tree *tree, struct bplus_node *node,
                                   ...
                                   uint64_t key, int insert)
{
    memmove(&key(node)[insert + 1], &key(node)[insert], (node->children - 1 - insert) * sizeof(uint64_t));
                             ...
                             struct bplus_node *left, uint64_t key, value_t data, int insert)
{
    ...
    memmove(&key(left)[0], &key(leaf)[0], pivot * sizeof(uint64_t));
    memmove(&data(left)[0], &data(leaf)[0], pivot * sizeof(value_t));
    ...
    memmove(&key(left)[pivot + 1], &key(leaf)[pivot], (split - pivot - 1) * sizeof(uint64_t));
    memmove(&data(left)[pivot + 1], &data(leaf)[pivot], (split - pivot - 1) * sizeof(value_t));

    /* original leaf left shift */
    memmove(&key(leaf)[0], &key(leaf)[split - 1], leaf->children * sizeof(uint64_t));
    memmove(&data(leaf)[0], &data(leaf)[split - 1], leaf->children * sizeof(value_t));
                              ...
                              struct bplus_node *right, uint64_t key, value_t data, int insert)
{
    ...
    memmove(&key(right)[0], &key(leaf)[split], pivot * sizeof(uint64_t));
    memmove(&data(right)[0], &data(leaf)[split], pivot * sizeof(value_t));
    ...
    memmove(&key(right)[pivot + 1], &key(leaf)[insert], (_max_entries - insert) * sizeof(uint64_t));
    memmove(&data(right)[pivot + 1], &data(leaf)[insert], (_max_entries - insert) * sizeof(value_t));

    return key(right)[0];
}

static void leaf_simple_insert(struct bplus_tree *tree, struct bplus_node *leaf,
                               uint64_t key, value_t data, int insert)
{
    memmove(&key(leaf)[insert + 1], &key(leaf)[insert], (leaf->children - insert) * sizeof(uint64_t));
    memmove(&data(leaf)[insert + 1], &data(leaf)[insert], (leaf->children - insert) * sizeof(value_t));
    key(leaf)[insert] = key;
    data(leaf)[insert] = data;
    leaf->children++;
}

static int leaf_insert(struct bplus_tree *tree, struct bplus_node *leaf, uint64_t key, value_t data)
{
    /* Search key location */
                                     ...
                                     int parent_key_index, int remove)
{
    ...
    memmove(&key(node)[1], &key(node)[0], remove * sizeof(uint64_t));
                                     ...
                                     int parent_key_index, int remove)
{
    ...
    memmove(&key(left)[left->children], &key(node)[0], remove * sizeof(uint64_t));
    memmove(&sub(left)[left->children], &sub(node)[0], (remove + 1) * sizeof(off_t));

    /* sub-node sum = node->children - 1 */
    memmove(&key(left)[left->children + remove], &key(node)[remove + 1], (node->children - remove - 2) * sizeof(uint64_t));
                                      ...
                                      int parent_key_index)
{
    ...
    memmove(&key(right)[0], &key(right)[1], (right->children - 2) * sizeof(uint64_t));
                                      ...
                                      int parent_key_index)
{
    ...
    memmove(&key(node)[node->children - 1], &key(right)[0], (right->children - 1) * sizeof(uint64_t));
...
static inline void non_leaf_simple_remove(struct bplus_tree *tree, struct bplus_node *node, int remove)
{
    ...
    memmove(&key(node)[remove], &key(node)[remove + 1], (node->children - remove - 2) * sizeof(uint64_t));
                                 ...
                                 int parent_key_index, int remove)
{
    ...
    memmove(&key(leaf)[1], &key(leaf)[0], remove * sizeof(uint64_t));
                                 ...
                                 struct bplus_node *left, int parent_key_index, int remove)
{
    ...
    memmove(&key(left)[left->children], &key(leaf)[0], remove * sizeof(uint64_t));
    memmove(&data(left)[left->children], &data(leaf)[0], remove * sizeof(off_t));
    memmove(&key(left)[left->children + remove], &key(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(uint64_t));
                                  ...
                                  int parent_key_index)
{
    ...
    memmove(&key(right)[0], &key(right)[1], (right->children - 1) * sizeof(uint64_t));
...
{
    memmove(&key(leaf)[leaf->children], &key(right)[0], right->children * sizeof(uint64_t));
...
{
    memmove(&key(leaf)[remove], &key(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(uint64_t));
    memmove(&data(leaf)[remove], &data(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(off_t));
    leaf->children--;
}

static int leaf_remove(struct bplus_tree *tree, struct bplus_node *leaf, uint64_t key)
{
...
static int bplus_tree_delete(struct bplus_tree *tree, uint64_t key)
{
    struct bplus_node *node = node_seek(tree, tree->root);
    while (node != NULL) {
        if (is_leaf(node)) {
