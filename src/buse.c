...
//    fprintf(stdout, "bbbb\n");
    ...
    /* TODO: SET_BLKSIZE and SET_SIZE_BLOCKS not used currently */
//    if (aop->blocksize) {
//        err = ioctl(nbd, NBD_SET_BLKSIZE, aop->blocksize);
//        assert(err != -1);
//    }
//    if (aop->blocksize) {
//        err = ioctl(nbd, NBD_SET_SIZE_BLOCKS, aop->blocksize);
//        assert(err != -1);
//    }

    // only NBD_SET_SIZE is available now
    if (aop->size) {
        err = ioctl(nbd, NBD_SET_SIZE, aop->size);
        assert(err != -1);
    }
    /* TODO: NBD_SET_SIZE is not advisable */
	...
	if (!fork()) {
		...
		ioctl(nbd, NBD_CLEAR_SOCK);
		printf("child pthread init success\n");
	}
...
//                fprintf(stderr, "magic: %d", ntohl(cr->magic));


				if (ntohl(cr->type) == NBD_CMD_WRITE) {
					/* FIXME: why handle WRITE REQUEST there? why not READ REQUEST */
					len = ntohl(cr->len);
					//assert(aop->write);
					cr->buf = malloc(len);
                ...
                if (ses.go_on) {
                    /* FIXME: when thread pool is full, we should expand it */
